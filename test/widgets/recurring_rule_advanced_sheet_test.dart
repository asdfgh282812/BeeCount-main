...
void main() {
  Widget host(void Function(RecurringRuleDraft? result) onResult,
      {DateTime? anchorDate}) {
    return MaterialApp(
  ...
  testWidgets('切到「週期」+「月」頻率,確定後回傳 monthly_day draft(預設用 anchorDate 的日）',
    ...
    expect(result!.advancedRule, {'type': 'monthly_day', 'day': 10});
    expect(result!.endAt, isNull); // 預設「無限期」
  });
  ...
  testWidgets('切到「週期」+「週」頻率,切換星期 chip 後回傳 weekly_days draft', (tester) async {
    ...
    final days = (result!.advancedRule!['days'] as List).cast<int>()..sort();
  ...
  testWidgets('指定次數模式:點次數數字叫出鍵盤輸入,新次數反映到 endAt', (tester) async {
    ...
    expect(result!.endAt, DateTime(2026, 5, 10));
  });
  ...
  testWidgets('截止日期模式:預設帶入 anchorDate,不額外選日期時 endAt 等於 anchorDate',
    ...
    expect(result, isNotNull);
    expect(result!.endAt, DateTime(2026, 3, 10));
  });
